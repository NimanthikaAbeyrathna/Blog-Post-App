<!DOCTYPE html>
<html lang="en" class="scroll-smooth">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>About</title>
  <link rel="icon" type="image/png" href="./images/favicon.png" />
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-50 text-gray-800 font-sans">

    <?php include "nav.php" ?>

    <div class="max-w-3xl mx-auto p-6 mt-12 space-y-8 text-center">
    
    <h1 class="text-4xl font-bold text-indigo-700">About This Blog</h1>
    
    <img src="./images/myphoto.png" alt="" class="mx-auto rounded-full w-40 h-40 object-cover shadow-lg border-4 border-indigo-300"/>
    
    <p class="text-lg italic text-gray-600 leading-relaxed">
        “This blog is where I share my book reviews, science experiments, and fun learning tips!”
    </p>
    
    <p class="text-md text-gray-700">
        “Feel free to explore and comment on your favorite posts!”
    </p>
    
    </div>

    <?php include "footer.php" ?>

</body>
</html>
